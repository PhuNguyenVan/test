 ...
 * @return int
 */
function wc_get_campaign_id_by_code( $code, $exclude = 0 ) {
	$data_store = WC_Campaign_Data_Store::load( 'campaign' );
	$ids        = wp_cache_get( WC_Cache_Helper::get_cache_prefix( 'campaigns' ) . 'campaign_id_from_code_' . $code, 'campaigns' );

	if ( false === $ids ) {
		$ids = $data_store->get_ids_by_code( $code );
		if ( $ids ) {
			wp_cache_set( WC_Cache_Helper::get_cache_prefix( 'campaigns' ) . 'campaign_id_from_code_' . $code, $ids, 'campaigns' );
		}
	}

	$ids = array_diff( array_filter( array_map( 'absint', (array) $ids ) ), array( $exclude ) );

	return apply_filters( 'woocommerce_get_campaign_id_from_code', absint( current( $ids ) ), $code, $exclude );
}
