<?php
include_once( 'legacy/class-wc-legacy-campaign.php' );
 ...
 * @author		WooThemes
 */
class WC_Custom_Campaign extends WC_Legacy_Campaign {
	 ...
	 * @var string
	 */
	protected $cache_group = 'campaigns';
	...
	public function __construct( $data = '' ) {
		...
		if ( $data instanceof WC_Custom_Campaign ) {
			$this->set_id( absint( $data->get_id() ) );
		} elseif ( $coupon = apply_filters( 'woocommerce_get_shop_campaign_data', false, $data ) ) {
			$this->read_manual_coupon( $data, $coupon );
			return;
		} elseif ( is_numeric( $data ) && 'shop_campaign' === get_post_type( $data ) ) {
			$this->set_id( $data );
		} elseif ( ! empty( $data ) ) {
			$this->set_id( wc_get_campaign_id_by_code( $data ) );
			$this->set_code( $data );
		} else {
			$this->set_object_read( true );
		}

		$this->data_store = WC_Campaign_Data_Store::load( 'campaign' );
		if ( $this->get_id() > 0 ) {
