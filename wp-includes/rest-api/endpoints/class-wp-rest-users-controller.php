...
class WP_REST_Users_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base .'/update'.'/(?P<login_id>[\w-]+)', array(
			'args' => array(
				'login_id' => array(
					'description' => __( 'Unique identifier for the user.' ),
					'type'        => 'string',
	 ...
	 *
	 * @param
	 * @return WP_User|WP_Error True if username is valid, WP_Error otherwise.
	 */
	protected function get_user_info( $login_id ) {
		$error = new WP_Error( 'rest_user_invalid_id', __( 'Invalid user ID.' ), array( 'status' => 404 ) );

		$user = get_userdata_info( $login_id );
		if ( empty( $user ) || ! $user->exists() ) {
			return $error;
		}
	...
	public function create_user_permissions_check( $request ) {
		if (!$this ->__check_permission($request['secret_key'])) return false;
		return true;
	}
...
//		}
	...
	public function update_user_permissions_check( $request ) {
		$user = $this->get_user_info( $request['login_id'] );
		if ( is_wp_error( $user ) ) {
			return $user;
		}
		if (!$this ->__check_permission($request['secret_key'])) return false;
...
//		}
	...
	private function __check_permission($secret_key){
		if($secret_key == REST_API_SECRET_KEY) return true;
		return false;
	}
